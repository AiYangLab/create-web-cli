 ...
 * 项目配置的交互式提示
 */
export async function prompt(projectName: string | undefined, options: PromptOptions): Promise<ProjectConfig> {
  // 获取可用的框架
  ...
  const frameworkAnswer = await inquirer.prompt([
    {
      ...
      choices: frameworks.map(fw => {
        ...
        return {
          name: `${frameworkInfo?.name || fw} - ${frameworkInfo?.description || ''}`,
          value: fw
        };
      }),
      default: options.template || (frameworks.includes('react') ? 'react' : frameworks[0])
    }
  ]);
  ...
  const buildToolChoices = availableBuildTools.map(tool => {
    ...
    return {
      name: `${buildToolInfo?.name || tool} - ${buildToolInfo?.description || ''}`,
      value: tool
    };
  });
  ...
  const answers = await inquirer.prompt([
    ...
    },
    {
      ...
      default: false,
      when: (answers) => answers.eslint || answers.prettier || answers.commitizen
    },
    {
      type: 'list',
      name: 'packageManager',
