...
export function generateLintStagedConfig(options: { 
  ...
  const extensions = typescript ? 
    (framework === 'vue' ? 'js,jsx,ts,tsx,vue' : 'js,jsx,ts,tsx') : 
    (framework === 'vue' ? 'js,jsx,vue' : 'js,jsx');
  ...
  // 其他文件类型的 Prettier
  if (prettier) {
    lintStagedContent += '  "{!(package)*.json,*.code-snippets,.!(browserslist)*rc}": ["prettier --write--parser json"],\n';
  ...
  // Stylelint
  if (stylelint) {
    const styleExtensions = framework === 'vue' ? 'scss,less,styl,html,vue' : 'scss,less,styl,html';
