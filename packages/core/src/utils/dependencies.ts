...
export function getProjectDependencies(options: {
  ...
  const { framework, typescript, buildTool, ui, styling, linting, git } = options;
  ...
  // Git tools
  devDependencies.push(...getGitDependencies({
...
export function generatePackageJson(options: {
  ...
  const { name, dependencies, devDependencies, framework, buildTool, typescript } = options;
